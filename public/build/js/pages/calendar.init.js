...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleEventClicked(event) {
        ...
        if (
            selectedEvent.extendedProps.is_enable_event_registration &&
            new Date(selectedEvent.start) >= new Date() &&
            event_section_ids.includes(user_section_id)
        ) {
            document
                .getElementById("register-event-btn")
                .classList.remove("d-none");
            document
                .getElementById("register-event-btn")
                .classList.add("d-block");
            document
                .getElementById("registration-failed-message")
        ...
        document
            .getElementById("register-event-btn")
            .setAttribute("data-event-id", selectedEvent.id);
        ...
        document.getElementById("event-location-field").value =
            selectedEvent.extendedProps.location === undefined
                ? "No Location"
                : selectedEvent.extendedProps.location;
        document.getElementById("latitude-field").value =
            selectedEvent.extendedProps.latitude;
        document.getElementById("longitude-field").value =
            selectedEvent.extendedProps.longitude;
    ...
    function memberEvents(data) {
        ...
        if (data.events_for_you.length > 0) {
            ...
            let upComingEventCards = document.querySelectorAll(
                ".upcoming-event-card",
            );
    ...
    const fetchUpcomingEvents = async () => {
        if (document.querySelector("#upcoming-event-list")) {
            const response = await fetch(
                `/dashboard/events/all?filter=upcoming_events`,
            );
            const data = await response.json();
    ...
    const fetchMemberEvents = async () => {
        if (document.querySelector("#member-events-tab")) {
            const response = await fetch(
                "/dashboard/events/all?filter=member_events",
            );
            const data = await response.json();
            memberEvents(data);
        }
    ...
    $("#btn-delete-event").on("click", function (e) {
        ...
        showDeleteMessage({
            message:
                '<strong class="text-danger">Removing this event</strong> will remove all of the information from our database.',
    ...
    function upcomingEvent(a) {
        ...
        Array.from(a).forEach(function (element) {
            ...
            if (element.end) {
                ...
                var updatedDay = endUpdatedDay.setDate(
            ...
            u_event =
                "<div class='card mb-3 upcoming-event-card' data-event-id='" +
                element.id +
                "'>\
                            <div class='card-body'>\
                                <div class='d-flex justify-content-between mb-3'>\
                                    <div class='flex-shrink-0 fs-12'>" +
                str_dt(element.start_date) +
                "</div>\
                                    <div class='badge bg-info-subtle text-info' style='font-size: 10px;'>" +
                $type +
                "</div>\
                                    <div class='flex-shrink-0'><small class='badge bg-primary-subtle text-primary ms-auto'>" +
                tConvert(element.time) +
                "</small></div>\
                                </div>\
                ...
                (element.area != null
                    ? element.area.replace("_", " ").toUpperCase()
                    : " ") +
                "</div>\
                                <p class='text-muted text-truncate-two-lines mt-1'> " +
                element.location +
                "</p>\
                            </div>\
                        </div>";
...
function eventClicked() {
    ...
    document
        ...
        .classList.replace("d-block", "d-none");
    document
        .getElementById("event-location-field")
        .classList.replace("d-block", "d-none");
    document
